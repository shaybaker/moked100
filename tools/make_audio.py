...
LINES = {
    ...
    "done_home_burglary": (FIELD, "ניידת עשרים ואחת עֲבוֹר מוקד. הפורץ נעצר בתוך הדירה, כל החפצים חזרו לבעלים. האירוע סגור."),
    "done_shop_breakin": (FIELD, "עֲבוֹר מוקד, תפסנו את הפורץ ליד דלת החנות לפני שהספיק להיכנס. סגור."),
    "done_car_breakin": (FIELD, "עֲבוֹר מוקד, הפורץ נתפס בקומה שתיים בחניון. בעל הרכב עודכן, האירוע סגור."),
    "done_bag_snatch": (FIELD, "אופנוע שבע עֲבוֹר מוקד. השגנו את החוטף אחרי מרדף קצר, התיק חזר לבעלים. סגור."),
    "done_lost_child": (FIELD, "קהילתי שתים עשרה עֲבוֹר מוקד. נשארתי עם הילד עד שההורים הגיעו, הכל בסדר. סגור."),
    "done_cat_tree": (FIELD, "עֲבוֹר מוקד, הרחקנו את הכלב והורדנו את החתול בשלום. הילדה מחייכת. סגור."),
    "done_red_light": (FIELD, "תנועה ארבע עֲבוֹר מוקד. עצרנו את המונית, הנהג קיבל דוח על נסיעה באור אדום. סגור."),
    "done_pickpocket": (FIELD, "עֲבוֹר מוקד, איתרנו את הכייס ליד היציאה מהקניון. הטלפון חזר לאישה. סגור."),
    "done_phone_snatch": (FIELD, "אופנוע שבע עֲבוֹר מוקד. תפסנו את החוטף ליד הכביש, הטלפון חזר לבעלים. סגור."),
    "done_window_smash": (FIELD, "עֲבוֹר מוקד, המנפץ נעצר בחניון. בעלי הרכבים עודכנו. סגור."),
    "done_prowler": (FIELD, "עֲבוֹר מוקד, איתרנו את החשוד עם הפנס בין הבתים. הוא נלקח לתחנה לבירור, השכונה שקטה. סגור."),
    "done_tire_thief": (FIELD, "עֲבוֹר מוקד, תפסנו את הגנב עם הגלגל ביד. הגלגל חזר לרכב. סגור."),
    "done_hotel_burglar": (FIELD, "עֲבוֹר מוקד, הפורץ נעצר בתוך החדר בקומה שלוש. האורחים קיבלו את החפצים שלהם. סגור."),
    "done_field_fire": (FIELD, "כבאית מאה ושתיים עֲבוֹר מוקד. השריפה בחורשה כובתה, אף בית לא נפגע. סגור."),
    "done_warehouse_fire": (FIELD, "כבאית עֲבוֹר מוקד. האש במחסן בשליטה וכובתה. אין נפגעים. סגור."),
    "done_tree_on_car": (FIELD, "תנועה ארבע עֲבוֹר מוקד. חסמנו את הרחוב, פינינו את העץ, והכביש פתוח. סגור."),
    "done_suspicious_bag": (FIELD, "עֲבוֹר מוקד, הרחקנו את הנוסעים והחבלן בדק את התיק. התברר שזה תיק של תייר שהלך לאכול. סגור."),
    "done_lost_dog": (FIELD, "קהילתי שתים עשרה עֲבוֹר מוקד. הכלב איתי, על הקולר היה מספר טלפון. הבעלים בדרך. סגור."),
    "done_kid_on_road": (FIELD, "עֲבוֹר מוקד, הילדה ירדה מהכביש למדרכה בשלום. ההורים אותרו ועודכנו. סגור."),
    "done_speeding_moto": (FIELD, "תנועה ארבע עֲבוֹר מוקד. עצרנו את האופנוען במחלף. הרישיון נשלל והוא קיבל דוח. סגור."),
    "done_car_accident": (FIELD, "אמבולנס מאה ואחת עֲבוֹר מוקד. הנהג קיבל טיפול ביד, הוא במצב קל. פינינו את הכביש. סגור."),
    "done_tunnel_rescue": (FIELD, "כבאית עֲבוֹר מוקד. חילצנו את הנהג מהרכב, הוא בסדר גמור. המנהרה נפתחה לתנועה. סגור."),
    "done_missing_hiker": (FIELD, "כלבן תשע עֲבוֹר מוקד. הכלב מצא את המטייל ליד הנחל, קצת רטוב אבל בריא. סגור."),
}
...
NAMES = {
    # key: (as typed on the login screen, spoken form). Nikud only where the voice needs it (איתי, אלה,
    # אביגיל, אמה are misread without it); avoid dagesh on letters other than ב/כ/פ, the voice turns it into an artefact.
    "itay": ("איתי", "אִיתַי"), "david": ("דוד", "דָוִד"), "uri": ("אורי", "אוּרִי"), "yosef": ("יוסף", "יוֹסֵף"),
    "ariel": ("אריאל", "אֲרִיאֵל"), "daniel": ("דניאל", "דניאל"), "noam": ("נועם", "נועם"), "eitan": ("איתן", "אֵיתָן"),
    ...
    "itamar": ("איתמר", "אִיתָמָר"), "alon": ("אלון", "אַלוֹן"), "nadav": ("נדב", "נָדָב"), "guy": ("גיא", "גַיְא"),
    "roee": ("רועי", "רוֹעִי"), "tal": ("טל", "טַל"), "ilay": ("עילאי", "עִילַאי"), "liam": ("ליאם", "לִיאָם"),
    "yuval": ("יובל", "יובל"), "noa": ("נועה", "נֹעָה"), "tamar": ("תמר", "תָמָר"), "maya": ("מאיה", "מָאיָה"),
    "avigail": ("אביגיל", "אֲבִיגַיִל"), "shira": ("שירה", "שִׁירָה"), "yael": ("יעל", "יָעֵל"), "adel": ("אדל", "אָדֵל"),
    "ayala": ("איילה", "איילה"), "talia": ("טליה", "טַלְיָה"), "lia": ("ליה", "ליה"), "romi": ("רומי", "רוֹמִי"),
    "ella": ("אלה", "אֵלָה"), "hila": ("הילה", "הִילָה"), "michal": ("מיכל", "מיכל"), "agam": ("אגם", "אֲגַם"),
    "sara": ("שרה", "שָׂרָה"), "naama": ("נעמה", "נַעֲמָה"), "gali": ("גלי", "גַלִי"), "emma": ("אמה", "אֶמָה"),
    "libi": ("ליבי", "לִיבִּי"), "shai": ("שי", "שַׁי"), "dana": ("דנה", "דָנָה"), "amit": ("עמית", "עָמִית"),
